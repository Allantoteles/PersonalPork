...
import { useRutinasAtleta, useRoutineImages } from '../../lib/turso-hooks';
            ...
            <div key={dia} className="mb-8">
              ...
              <div className="flex flex-col gap-3">
                {rutinasDia.map((routine) => (
                  ...
                  >
                    <RoutineImage rutinaVersionId={routine.id} />
...
function RoutineImage({ rutinaVersionId }: { rutinaVersionId: string }) {
  const { data: images = [] } = useRoutineImages(rutinaVersionId);
  ...
  return (
    <div className="w-24 h-24 shrink-0 overflow-hidden">
      <div className={`w-full h-full grid ${gridClass} gap-0.5`}>
        {displayImages.map((img, index) => (
          <div key={`${img.id}-${index}`} className="bg-[#2f3638] overflow-hidden">
            <img
              src={img.imagen}
              alt=""
              className="w-full h-full object-cover"
