...
import { useAlumnoDb, useAlumnoRutinas } from '../../../lib/turso-hooks';
...
export default function AlumnoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loadingAlumno || loadingRutinas) {
    return (
          ...
          {rutinasPorDia.map(({ dia, rutinas: rutinasDia }) => (
            <div key={dia} className="mb-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <Link
                  href={`/rutinas?alumno=${id}&dia=${encodeURIComponent(dia)}`}
                  ...
                  {rutinasDia.map(rutina => (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <Link
                          href={`/rutinas?alumno=${id}&version=${rutina.id}`}
                          className="p-2 bg-[#242b2d] rounded-lg active:scale-95 transition-transform"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#ff6b00]">
